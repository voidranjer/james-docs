...
### The issue of train loss being larger than validation
...
A **higher** training loss over validation loss means that when we feed the model with data used in the training, the model is not able to predict the output as well as it does with **validation data** (not seen by the model during training), which is unusual.

As Alex mentioned, this could be due to random sampling or other factors in the model that are causing this behavior. To investigate, we'll try to overfit the model on a small subset of the training data (10 samples per category) and see if the validation loss overtakes the training loss.
